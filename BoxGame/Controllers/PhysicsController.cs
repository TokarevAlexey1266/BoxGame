...
namespace BoxGame.Controllers
{
    public static class PhysicsController
    {
        ...
        {
            if (entity.posX + dir.X <= 0 || entity.posX + dir.X >= mapController.cellSize * (mapController.mapWidth-1) || entity.posY + dir.Y <= 0 || entity.posY + dir.Y >= mapController.cellSize * (mapController.mapHeigh-1))
                return true;
            for (var i = 0; i < mapController.mapObjects.Count; i++)
            {
